...
% Inserting uncertainty to the descretized system while 
...
for N = 3:max_num_of_agents
    ...
    initial_cost{N-d+1} = lqr_cost(A_hat,B_hat,Q_hat,R_hat,K_hat);
    initial_ctr{N-d+1} = K_hat;
    ...
    final_cost{N-d+1} = lqr_cost(A_hat,B_hat,Q_hat,R_hat,K_hat_out);
    final_ctr{N-d+1} = K_hat_out;

    % LQR optimal policy which is not feasible
    [K_opt_unconst, P_opt_unconst] = dlqr(A_hat,B_hat,Q_hat,R_hat);
    K_hat_lqr_unfeas{N-d+1} = K_opt_unconst;
    P_hat_lqr_unfeas{N-d+1} = P_opt_unconst;
...
fig1 = figure;
hold on
plot((3:max_num_of_agents),100*(trace_p_final-trace_p_lqr)./trace_p_lqr,'LineWidth',4,'color','blue')
set(gca,'fontsize',30)
...
fig2 = figure;
hold on
plot((3:max_num_of_agents),(trace_p_final),'LineWidth',7,'color','blue')
plot((3:max_num_of_agents),(trace_p_initial),':','LineWidth',4,'color','black')
plot((3:max_num_of_agents),(trace_p_lqr),'-.','LineWidth',4,'color','red')
...
% saving files
saveas(fig1,'figures/Exp2-fig1','epsc')
saveas(fig2,'figures/Exp2-fig2','epsc')
clear fig1 fig2
Filename = sprintf('data/Exp2_%s', datestr(now,'mm-dd-yyyy-HH-MM'));
save(Filename)
